...
import vn.lottefinance.landingpage.properties.DagorasProperties;
import vn.lottefinance.landingpage.properties.LoanProperties;
...
public class DagorasClient {
    ...
    public Object send(Object obj, Class clazz) {
        try {
            log.info("==================== " + dagorasProperties.getSendUrl() + " ====================");
            ...
            Request request = new Request.Builder()
                    .url(dagorasProperties.getSendUrl())
                    .post(requestBody)
                    .build();
    ...
    public Object sendValidate(Object obj, Class clazz) {
        try {
            log.info("==================== " + dagorasProperties.getValidateUrl() + " ====================");
            RequestBody requestBody = RequestBody.create(MediaType.parse("application/json; charset=utf-8"), ConverterUtil.toJsonString(obj));
            Request request = new Request.Builder()
                    .url(dagorasProperties.getValidateUrl())
                    .post(requestBody)
                    .build();
            ResponseBody responseBody = okHttpClient.newCall(request).execute().body();
            return ConverterUtil.fromJSON(clazz, responseBody.string());
